...
public class HeavyCrossbowItem extends CrossbowItem {
    ...
    public HeavyCrossbowItem(Properties prop, Tiers material) {
        super(prop);
        this.modId = null;
        this.material = material;
        maxVelocity =  ABConfig.CONFIG.HeavyCrossbowProjectileVelocity.get().floatValue();
        loadTicks = ABConfig.CONFIG.heavyLoadTime.get();
        aimTicks = ABConfig.CONFIG.heavyAimTime.get();


        if (FMLEnvironment.dist.isClient()) {
            BowProperties.registerCrossbowProperties(this);
        }
    ...
    public static void performShooting(Level pLevel, LivingEntity pShooter, InteractionHand pUsedHand, ItemStack pCrossbowStack, float pVelocity, float pInaccuracy) {
        if (pShooter instanceof Player player && net.minecraftforge.event.ForgeEventFactory.onArrowLoose(pCrossbowStack, pShooter.level, player, 1, true) < 0)
            return;
        ...
        for(int i = 0; i < list.size(); ++i) {
            ...
            int stackAimTicks = getAimTicks(pCrossbowStack);
            int inaccuracy = Mth.clamp(stackAimTicks - i, 0, stackAimTicks);
            if (inaccuracy != 0) {
                pInaccuracy = 2.0F * ((float) inaccuracy / (float) stackAimTicks);
            }
    ...
    public static int getAimTicks(ItemStack stack) {
        int i = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.VANISHING_CURSE, stack);
        ...
        if (isCharged(pStack) && !list.isEmpty()) {
            ...
            if (pFlag.isAdvanced() && itemstack.is(Items.FIREWORK_ROCKET)) {
                ...
                if (isShiftPressed) {
                    ...
                    pTooltip.add((Component.translatable(String.format("tooltip.archbows.heavy_crossbow.desc_5", "archbows")).withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.ITALIC));
                    pTooltip.add((Component.translatable(String.format("tooltip.archbows.heavy_crossbow.desc", "archbows")).withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.ITALIC));
                    pTooltip.add((Component.translatable(String.format("tooltip.archbows.heavy_crossbow.desc_4", "archbows")).withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.ITALIC));
